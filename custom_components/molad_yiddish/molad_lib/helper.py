...
Only dependency now is HA's built-in hdate[astral]==1.1.0.
"""
...
from hdate import Zmanim, Location  # used for Mevorchim logic
...
class MoladHelper:
    ...
    def carry_and_reduce(self, out0):
        ...
        # days (7 per week, keep Shabbos=7)
        xx = out0[0] + zz
        yy = (xx + 6) % 7 + 1
        if yy < 0:
            yy += 7
    ...
    def get_actual_molad(self, date):
        ...
        regular = sum(1 for i in range(yrs-1) if i+1 not in guachadazat)
        leap    = sum(1 for i in range(yrs-1) if i+1 in guachadazat)

        multipliers = [1, cycles, regular, leap, month-1]
    ...
    def is_shabbos_mevorchim(self, date) -> bool:
        ...
        return ( self.is_actual_shabbat(z)
                 and hd==sm
